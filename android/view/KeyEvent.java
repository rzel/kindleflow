...
public class KeyEvent {
    ...
    public static int getDeadChar(int accent, int c) {
    	...
    	return 0;
    	// XXX return KeyCharacterMap.getDeadChar(accent, c);
    }
    ...
    public char getDisplayLabel() {
        return ' ';
    	// XXX return KeyCharacterMap.load(mDeviceId).getDisplayLabel(mKeyCode);
    }
    ...
    public int getUnicodeChar(int meta) {
        return 0;
    	// XXX return KeyCharacterMap.load(mDeviceId).get(mKeyCode, meta);
    }
    ...
    */
    ...
    public final boolean dispatch(Callback receiver, DispatcherState state, Object target) {
        switch (mAction) {
	        ...
	        {
                mFlags &= ~FLAG_START_TRACKING;
                if (DEBUG) Log.t(TAG, "Key down to " + target + " in " + state + ": " + this);
                boolean res = receiver.onKeyDown(mKeyCode, this);
                if (state != null) {
                    if (res && mRepeatCount == 0 && (mFlags&FLAG_START_TRACKING) != 0) {
                        if (DEBUG) Log.t(TAG, "  Start tracking!");
                    ...
                    /*
                       ...
                       if (receiver.onKeyLongPress(mKeyCode, this)) {
                    	   if (DEBUG) Log.t(TAG, "  Clear from long press!");
            ...
            case ACTION_UP:
                if (DEBUG) Log.t(TAG, "Key up to " + target + " in " + state + ": " + this);
                if (state != null) {
    ...
    public static class DispatcherState {
       ...
       public void startTracking(KeyEvent event, Object target) {
           ...
           if (DEBUG) Log.t(TAG, "Start trackingt in " + target + ": " + this);
      ...
      public void handleUpEvent(KeyEvent event) {
          ...
          /*
          if (DEBUG) Log.t(TAG, "Handle key up " + event + ": " + this);
          int index = mActiveLongPresses.indexOfKey(keyCode);
          if (index >= 0) {
              if (DEBUG) Log.t(TAG, "  Index: " + index);
